    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if nouveau_lieu.disponibilite=="Libre" and nouveau_lieu.id_equip=="Mangeoire" and animal.etat=='Affamé':
                ...
                nouveau_lieu.save()
                messages.add_message(request, messages.INFO, 'Animal déplacé dans la mangeoire')
                ...
                nouveau_lieu.disponibilite = "Occupé"
                nouveau_lieu.save()
                messages.add_message(request, messages.INFO, 'Animal déplacé dans la roue')
                ...
                nouveau_lieu.disponibilite = "Occupé"
                nouveau_lieu.save()
                messages.add_message(request, messages.INFO, 'Animal déplacé dans le nid')
                ...
                nouveau_lieu.save()
                messages.add_message(request, messages.INFO, 'Animal déplacé dans la litière')
            else :
                print('message')
                messages.add_message(request, messages.INFO, 'Impossible de déplacer cet animal là !')
...
def ajout_animal(request): 
    ...
    if request.method == 'POST': 
        ...
        if form.is_valid(): 
            form.save() 
            messages.add_message(request, messages.INFO, 'Animal ajouté ! ')
